...
def get_conv2d_output_shape(input, weight, stride, padding):
    d_filter, h_filter, w_filter = weight.size()
    n_x, d_x, h_x, w_x = input.size()

    h_out = (h_x - h_filter + 2 * padding) // stride + 1
    w_out = (w_x - w_filter + 2 * padding) // stride + 1

    return (n_x, d_filter, h_out, w_out)
...
class DepthAdder2DFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx,
                input,
                weight,
...
class DepthAdder2D(nn.Module):
    def __init__(self,
        ...
        super(DepthAdder2D, self).__init__()
        self.stride = stride
        ...
        self.depth_adder = torch.nn.Parameter(
            nn.init.normal_(torch.randn(
               input_channel,3,3)))
        self.qdepth_adder = None
        self.bias = bias
        if bias:
        ...
        #self.register_buffer('depth_adder_mask', torch.Tensor(*self.depth_adder.size()).float())
        #self.set_mask()

 #       if self.quantize is True:
 #           print(self.quantize)
 #           print('quantize depth_adder layer to {} bits.'.format(self.weight_bits))

    def forward(self, input):
        #if self.sparsity != 0:
            # apply mask
        #weight = self.depth_adder.data
        #l1_weight = torch.norm(weight, dim=1)
        #depth_adder_data = weight + 0.1*l1_weight
            #self.depth_adder_mask.data = self.set_mask(self.depth_adder.data)
#        self.depth_adder.data =self.depth_adder.data * self.depth_adder_mask.data

        if self.quantize is True:
            #shift_range = (-1 * (2 ** (self.weight_bits - 1) - 1), 0)
            #self.depth_adder.data = ste.clampabs(self.depth_adder.data, 2**shift_range[0], 2**shift_range[1])
            #weight_q = ste.round_power_of_2(self.depth_adder, 'deterministic')

            # quantization v1
            if self.quantize_v == 'wageubn':
                self.qdepth_adder = round_weight_fixed_point(self.depth_adder, self.weight_bits)
            ...
            if self.quantize_v == 'sbm':
                ...
                weight_qparams = calculate_qparams(self.depth_adder, num_bits=self.weight_bits, flatten_dims=(1, -1), reduce_dim=None)
                self.qdepth_adder = quantize(self.depth_adder, qparams=weight_qparams)
            ...
            output = DepthAdder2DFunction.apply(input,
                                           self.depth_adder,
                                           self.kernel_size,
                                           self.stride,
                                           self.padding,
    ...
    def set_mask(self):
        # random fix zero
        # min_add = self.depth_adder.data
        self.depth_adder_mask.data = (self.s > self.sparsity).float()
        assert self.depth_adder_mask.requires_grad == False
