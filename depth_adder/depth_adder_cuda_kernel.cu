...
__global__ void depth_adder_forward_kernel(
    ...
    const int PW, const int PH)
{
    ...
    {
        const int n = index / OW / OH / out_channels; // Batch Index
        const int m = index / OW / OH % out_channels; // Output Channel Index
        const int h = index / OW % OH;
        const int w = index % OW;

        const scalar_t *p_weight = weight + m * KH * KW;
...
template <typename scalar_t>
__global__ void depth_adder_backward_grad_in_kernel(
    scalar_t *grad_out,
    scalar_t *input,
    scalar_t *weight,
    scalar_t *grad_in,
...
template <typename scalar_t>
__global__ void depth_adder_backward_grad_weight_kernel(
    scalar_t *grad_out,
    scalar_t *input,
    scalar_t *weight,
    ...
    const int PW, const int PH)
{
    ...
    for (int batchIdx = batch; batchIdx < batch_size; batchIdx += nwarps)
    {
        ...
        for (int idx = laneId; idx < imageElements; idx += WARP_SIZE)
        {
            ...
            int outputOffset = ((batchIdx * out_channels + ch) * OH) * OW + idx;
    ...
    if (threadIdx.x == 0)
    {
        int weightOffset = kW + (KW * kH) + (KW * KH * ch);
    ...
    int PW, int PH)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(output.type(), "depth_adder_cuda_forward", ([&] {
                                   depth_adder_forward_kernel<scalar_t><<<num_blocks, NUM_THREADS>>>(
                                       input.data<scalar_t>(),
    ...
    int PW, int PH)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(grad_in.type(), "depth_adder_cuda_backward_grad_in", ([&] {
                                   depth_adder_backward_grad_in_kernel<scalar_t><<<num_blocks, NUM_THREADS>>>(
    ...
    int PW, int PH)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(grad_weight.type(), "depth_adder_cuda_backward_grad_weight", ([&] {
                                   depth_adder_backward_grad_weight_kernel<scalar_t><<<grid, block, block.x * sizeof(scalar_t)>>>(
                                       grad_out.data<scalar_t>(),
                                       input.data<scalar_t>(),
                                       weight.data<scalar_t>(),
